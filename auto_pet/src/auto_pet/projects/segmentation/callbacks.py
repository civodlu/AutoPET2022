...
import os
import trw
from auto_pet import was_started_within_vscode
from auto_pet import CallbackInference, find_root_sequence
from corelib import inference_process_wholebody_3d, test_time_inference

from auto_pet.projects.segmentation.metrics import calculate_metrics_autopet
...
def default_pre_training_callbacks(configuration, model):
    ...
    callbacks += [
        ...
        #trw.callbacks.CallbackReportingExportSamples(table_name='random_samples'),
        #trw.callbacks.CallbackProfiler(),
        #trw.callbacks.CallbackReportingAugmentations(),
        #trw.callbacks.CallbackLearningRateFinder()
    ]

    return callbacks


class ModelWithHighestMetric(trw.callbacks.callback_save_last_model.ModelWithLowestMetricBase):
    ...
    def update(self, metric_value, model, metadata, root_path):
        if metric_value is not None and metric_value >= self.minimum_metric and metric_value >= self.best_highest_metric:
            ...
            from trw.train import TrainerV2
...
def default_per_epoch_callbacks(configuration, model):
    ...
    callbacks = [
        ...
        trw.callbacks.CallbackReportingLearningRateRecorder(),
        trw.callbacks.CallbackSkipEpoch(
            ...
            callbacks=[
                ...
                trw.callbacks.CallbackSaveLastModel(keep_model_with_best_metric=ModelWithHighestMetric(
    ...
    if rolling_checkpoints is not None:
        callback = trw.callbacks.CallbackSkipEpoch(
...
def default_post_training_callbacks(configuration, model):
    ...
    callbacks += [
        ...
        trw.callbacks.CallbackExportHistory(),
        CallbackWriteReport(),
    ]
