...
from trw.layers import ModuleWithIntermediate
from trw.layers import ConvBlockType, BlockConvNormActivation
...
class ModelUNetMulticlassDeepSupervision(nn.Module):
    def __init__(
        ...
        self.output_fn = partial(trw.train.OutputSegmentation, criterion_fn=self.loss_fn, output_postprocessing=self.output_postprocessing)
        
        assert int(with_deep_supervision) + int(with_deep_supervision_composite) <= 1 
        if with_deep_supervision:
            self.model = trw.layers.DeepSupervision(
                model, 
    ...
    def forward(self, batch: Batch) -> Dict:
        ...
        if seg is None:
            ...
            return {
                'seg': trw.train.OutputEmbedding(o)
            }
            ...
            if intermediates is not None:
                outputs['features'] = trw.train.OutputEmbedding(intermediates[-2])
        else:
            o = self.model(features)
            outputs = {
        ...
        if self.boundary_loss_factor is not None and dt is not None:
            ...
            outputs['boundary'] = trw.train.OutputLoss(boundary_loss)
...
class LossDiceCrossEntropyFocal2:
    def __init__(self, ce_factor=0.5, gamma=2.0, sensitivity=0.0) -> None:
        ...
        self.dice = trw.train.LossDiceMulticlass(normalization_fn=partial(nn.Softmax, dim=1), discard_background_loss=True, smooth=1e-3)
