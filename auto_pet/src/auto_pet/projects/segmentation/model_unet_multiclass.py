...
class ModelUNetMulticlass(nn.Module):
    ...
    def forward(self, batch: Batch) -> Dict:
        ...
        return {
            'seg': trw.train.OutputSegmentation(o, seg, criterion_fn=self.loss_fn, output_postprocessing=self.output_postprocessing),
            ...
            #'2d_found': trw.train.OutputEmbedding(self.output_postprocessing((o[:, :, z_half])).type(torch.float32)),
        }


class LossDiceCrossEntropyFocal:
    def __init__(self, ce_factor=0.5, gamma=2.0) -> None:
        self.focal = trw.train.LossFocalMulticlass(gamma=gamma)
        self.dice = trw.train.LossDiceMulticlass(normalization_fn=None, discard_background_loss=False, smooth=1.0)
...
SimpleMulticlassUNet = partial(
    ...
    loss_fn=partial(trw.train.LossDiceMulticlass, normalization_fn=None)
)
...
def make_resnet_backbone_decoder():
    ...
    encoder = trw.layers.PreActResNet(
                dimensionality=3, 
                input_channels=3, 
    ...
    decoder = trw.layers.BackboneDecoder(
        decoding_channels=(96, 64, 48, 32),
        output_channels=2,
        backbone=encoder,
        backbone_transverse_connections=(0, 1, 2, 3),
        backbone_input_shape=(1, 3, 96, 96, 96),
        middle_block_fn=partial(LatentConv, block=partial(trw.layers.BlockConvNormActivation, kernel_size=9)),
        config=config
    )
