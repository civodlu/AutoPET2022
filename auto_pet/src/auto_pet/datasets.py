...
from trw.train.sampler import SamplerRandom, SamplerSequential, SamplerSubsetRandomByListInterleaved
from trw.train.sequence_array import SequenceArray

from .transforms import transform_report_time
...
def load_case(
    ...
    full_path = os.path.join(data_root, data_version, path[0])
...
def create_datasets_inmemory_map(
        ...
        transform_test=None,
        max_jobs_at_once_factor=1) -> Dict[str, trw.train.Sequence]:
    """
...
def create_datasets_reservoir_map_weighted(
        ...
        max_jobs_at_once_factor=2,
        load_case_train_fn: Optional[Callable[[Dict, Namespace, List], Batch]] = None,
        load_case_valid_fn: Optional[Callable[[Dict, Namespace, List], Batch]] = None) -> Dict[str, trw.train.Sequence]:
    """
    Use a reservoir for the training where the data will be accumulated, then
    features are extracted using a map.
