try:
    ...
    print('TRW is not installed! See https://github.com/civodlu/trw for instuctions')
...
def inference_process_wholebody_2d(
    ...
    with torch.no_grad():
        ...
        for slice_n in range(half, shape[0] - half, slice_step_size):
            ...
            slices_input = trw.train.transfer_batch_to_device(slices_input, device)
...
def resize(v: TensorX, multiple_of: Union[int, Sequence[int]], padding_value: Number) -> Tuple[TensorNCX, ShapeX]:
    ...
    v_sub = trw.layers.crop_or_pad.crop_or_pad_fun(
    ...
    while z + tile_shape[0] <= shape_zyx[0]:
        ...
        while y + tile_shape[1] <= shape_zyx[1]:
            ...
            while x + tile_shape[2] <= shape_zyx[2]:
                ...
                model_device = trw.train.get_device(model)
                features = trw.train.transfer_batch_to_device(features, model_device)
                with torch.no_grad():
...
def inference_process_wholebody_3d(
    ...
    if tiling_strategy == 'none':
        model_device = trw.train.get_device(model)
        normalized_batch = trw.train.transfer_batch_to_device(normalized_batch, model_device)
        with torch.no_grad():
